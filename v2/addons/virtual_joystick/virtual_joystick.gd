...
enum Joystick_mode { FIXED, DYNAMIC, FOLLOWING }  ## The joystick doesn't move.  ## Every time the joystick area is pressed, the joystick position is set on the touched position.  ## When the finger moves outside the joystick area, the joystick will follow it.
...
enum Visibility_mode { ALWAYS, TOUCHSCREEN_ONLY, WHEN_TOUCHED }  ## Always visible  ## Visible on touch screens only  ## Visible only when touched
		...
		if event.pressed:
			if _is_point_inside_joystick_area(event.position) and _touch_index == -1:
				if (
					...
					or joystick_mode == Joystick_mode.FOLLOWING
					or (
...
func _update_joystick(touch_position: Vector2) -> void:
	...
	if (
		joystick_mode == Joystick_mode.FOLLOWING
		and touch_position.distance_to(center) > clampzone_size
	):
