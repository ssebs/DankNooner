...
Earn, spend and bank are three separate owners:
...
are `%BoostController:*` on the RollbackSynchronizer.

### The rule that bit us

netfox constrains **when** these vars may be written, not where they live.
...
`BoostController` runs first in that tick, ahead of every other controller, and is the only one that keeps evaluating while `is_crashed` — so a crash mid-boost cancels the burn instead of latching it (and its camera FX) until the respawn.
...
- **Spending** (`BoostController`) — drawn down alongside `boost_amount`. Without this, spending would leave a claim larger than the meter, and the next crash would eat previously banked boost too.

`PlayerEntity._apply_respawn_state()` then hands the claim to `BoostController.apply_crash_void()`, which subtracts it from `boost_amount` (clamped at zero). It must run **before** the `do_reset()` loop in that same function, which is what clears the combo state it depends on.
...
> Hook for flame VFX later: `BoostController.is_boosting` is synced, so remote players' flames work off the same bool.
