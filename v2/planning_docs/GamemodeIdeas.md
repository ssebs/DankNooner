# Gameplay & Modes

## Challenges (in-world, Skate-style) — replaces forced tutorials

> Players learn tricks by *playing*, not by being forced into a tutorial flow.

Challenges live in the world. Roll a motorcycle into a `ChallengeStartCircle` →
a 2D billboard speech bubble explains the task → complete it → success toast.
No NPCs, no dialog system, no gamemode swap — just opt-in mini-objectives
scattered through the open world.

See [PLAN-Challenge-system-tutorial.md](./PLAN-Challenge-system-tutorial.md)
for the implementation plan. Reuses `CheckPointMarker`, `SequentialTaskRunner`,
`TutorialHUD`, and the existing trick detection. New pieces: `SpeechBubble`
(2D billboard), `PerformTrickTask` (new `GameModeTask`), `ChallengeStartCircle`.

Examples of starter challenges:
- "Hold a wheelie for 5 seconds"
- "Stoppie into this zone"
- "Land a heel clicker off this ramp"
- "Pop a high chair while airborne"

Challenges also scaffold future modes: the same `PerformTrickTask` powers
Trick Battle scoring, and per-segment bonus tricks in races.

## Tutorial mode (legacy / onboarding)

The existing `TutorialGameMode` remains for true newbie onboarding (first-time
controls explanation). Most trick teaching is moving to the Challenge system
above. Possible future tutorial paths:

### Tutorial: The basics
> Learn the basics of riding a motorcycle to get your M1 license.

### Tutorial: Wheelies
> Learn how to clutch kick, and hold balance point

### Tutorial: Air Tricks
> Learn how to do classic FMX tricks

### Tutorial: Trick Tweaks
> Learn how to tweak your wheelies to get higher score

## Co-op gameplay
> *means that this is in MVP

### Free Roam in-lobby leaderboard*
- e.g. Longest Wheelie distance shows player + distance in lobby leaderboard
- Most points trick, fastest near miss, etc.

### Score Attack / Trick Battle*
- This is your "prove the core loop is fun" mode
- Timed round (60-90s), highest trick score wins
- You already have trick detection — just need a round timer + scoreboard
- Multiple locations that offer different ramps/possibilities, using the event_start_circles
- Ends with cops showing up, have to run away from them to keep earnings?

### Crash Launching*
- Players drag race & hit low fence, sending bodies flying.
- Furthest body wins

### Racing*
- Street - w/ traffic
- Sactioned Street - w/o traffic
- Stunt Race - w/ items

### Delivery driver*
- most deliveries w/ highest score
- start by delivering newspaper, then pizza, then pc parts, then ceramics/glass
- start mission to deliver x item in y time
- Doing tricks gains diff XP type (followers/respect), acts as multiplier for $
- XP to unlock bikes
- $ to unlock mods / skins


### Follow the Leader / H.O.R.S.E.
- One player is "it", rides wherever they want
- Other players:
  - must follow the same line
  - must match or beat it
- Turn-based within a round — very social, very replayable

### Cops & Robbers
- One team does tricks & does delivery missions, other team tries to crash into them as a police cop

### Club / coop story
- Friends all work for a company, deliver more dangerous items as you level up for more money


## Single player gameplay

### Rise to glory
> uses other modes as things to do, e.g. racing, delivery, stunt battles, etc. like Skate 3
> These progress the story
- See [Story.md](./Story.md)

### Delivery driver (same as above)

## Misc Ideas
- Add con to crashing, only 1 crash, or health bar, etc.
- Certain races are locked until you have certain class of bike
  - gear gating
  - e.g. must be A class to race
    - Could be base A fast bike
    - Could be base C modded bike
    - similar to Forza
- Running from the cops
  - More danger = more wanted level / Cop chases
  - they put up obstacles that you can stunt around
- Unlock mods to progress
- Surf like gamemode, replay rounds to get better on leaderboard. More time = more skill = higher rank
  - Adds replayability, can do it casually while listening to music/on toilet/etc.
  - E.g. flappy birds replayability
- Fuel mechanic, do tricks to get fuel. Level ends when you run out of fuel
- Make it a roguelite? But how will items work..
- Open world "menu level". If you want to do dirt bike race, go to that part of town to start, if you want track, go to the track, if you want to customize, go to the garage, etc.
- Endless mode like v1, maybe in an "arcade mode" that looks different from rest of the game
  - Or add endless highway